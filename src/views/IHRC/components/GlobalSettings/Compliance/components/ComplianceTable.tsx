    ...
    const fetchComplianceData = async (page: number, size: number) => {
        ...
        try {
            const { payload } = await dispatch(
                fetchCompliances({ page: page, page_size: size }),
            )
        ...
        if (itemToDelete?.id) {
            try {
                const response = await dispatch(
                    deleteCompliance(itemToDelete.id),
                )
                    .unwrap()
                    .catch((error: any) => {
    ...
    const columns = useMemo(
        () => [
            ...
            },
            {
                ...
                cell: (props) => (
                    <Tooltip title={props.getValue()} placement="top">
                        <div className="w-24 truncate uppercase">
                            {props.getValue()}
                        </div>
                    </Tooltip>
                ),
            ...
            },
            {
                header: 'Type',
                accessorKey: 'type',
                cell: (props) => <div className="w-28">{props.getValue()}</div>,
            },
            // {
            //   header: 'Clause',
            //   accessorKey: 'clause',
