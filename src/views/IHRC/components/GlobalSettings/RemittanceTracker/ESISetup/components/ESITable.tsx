...
import { Button, Tooltip, Dialog, DatePicker, Checkbox, toast, Notification } from '@/components/ui';
...
const ESITable = ({ refreshTrigger }) => {
  ...
  const handleEdit = async (config) => {
    try {
      ...
      setPaymentDueDates({
        firstDate: formatDate(detailData.esi_payment_due_date.first_date),
        secondDate: formatDate(detailData.esi_payment_due_date.second_date),
        thirdDate: formatDate(detailData.esi_payment_due_date.third_date),
...
console.log(validationErrors)
    ...
    try {
      ...
      await dispatch(updateESIConfig({ id: selectedState.value, data: esiConfigData }));
    ...
    <div className="relative">
      {esiTableData.length === 0 ? (
        <div className="flex flex-col items-center justify-center h-96 text-gray-500 border rounded-xl">
          ...
          <p className="text-center">No ESI Configurations Available</p>
        </div>
      ) : (
        <>
          <DataTable
            ...
            onPaginationChange={(page) => fetchESISetupData(page, tableData.pageSize)}
