...
const Posh = () => {
    ...
    const fetchPoshData = async () => {
        try {
            ...
            setPoshData(response.data.data || []);
            setTableData(prev => ({
                ...prev,
    ...
    useEffect(() => {
        fetchPoshData();
    }, [tableData.pageIndex, tableData.pageSize, searchTerm, timestamp]);
    ...
    const handleEdit = async (id: string) => {
        try {
            const response = await httpClient.get(endpoints.posh.detail(id));
            const poshData = response.data.data || response.data;
            
            console.log('Edit data:', poshData);
