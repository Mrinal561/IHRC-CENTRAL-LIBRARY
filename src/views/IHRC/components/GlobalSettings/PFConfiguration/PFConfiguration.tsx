...
const PFConfiguration = () => {
    ...
    const { pfConfigs, loading, error } = useSelector(
        (state: RootState) => state.pfconfig,
    )
    const [pfTableData, setPfTableData] = useState(false)
    ...
    const handleConfirm = async () => {
        try {
            if (isEditing && editingId) {
                await dispatch(
                    updatePFConfig({ id: editingId, data: newPF }),
                ).unwrap()
    ...
    const handleInputChange = (name: string, value: any) => {
        if (name === 'frequency') {
            ...
            setNewPF((prev) => ({
                ...prev,
                pf_frequency: value.toLowerCase() as
                    | 'monthly'
                    | 'half_yearly'
                    | 'yearly',
                pt_payment_due_date: {
                    ...
                    last_date:
                        value.toLowerCase() !== 'half_yearly'
                            ? null
                            : prev.pt_payment_due_date.last_date,
                },
            }))
    ...
    return (
        <AdaptableCard className="h-full" bodyClass="h-full">
            ...
            <Dialog isOpen={isDialogOpen} onClose={handleDialogClose}>
                <h5 className="mb-4">
                    {isEditing ? 'Edit' : 'Add'} PF Configuration
                </h5>
                <div className="flex flex-col gap-4">
                    ...
                    <div className="flex flex-col gap-2">
                        ...
                        <DatePicker
                            value={
                                newPF.pt_payment_due_date.first_date
                                    ? new Date(
                                          newPF.pt_payment_due_date.first_date,
                                      )
                                    : null
                            ...
                            onChange={(date) =>
                                handleInputChange('firstDueDate', date)
                            }
                    ...
                    <div className="flex flex-col gap-2">
                        ...
                        <DatePicker
                            value={
                                newPF.pt_payment_due_date.last_date
                                    ? new Date(
                                          newPF.pt_payment_due_date.last_date,
                                      )
                                    : null
                            ...
                            onChange={(date) =>
                                handleInputChange('secondDueDate', date)
                            }
